...
function cb(error, response, html) {
    if (error) {
        ...
        let allScorecardTags = document.querySelectorAll('.ds-grow.ds-px-4');
...
function cb2(error, response, html) {
    if (error) {
        ...
        const dom = new JSDOM(html);
        const document = dom.window.document;
        let batsmenRow = document.querySelectorAll('tbody [class="ds-w-0 ds-whitespace-nowrap ds-min-w-max"], .ds-border-line-primary');
